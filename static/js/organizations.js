  ...
  const app = new Vue({
    ...
    computed: {
      filteredData: function () {
        if (!(this.letter || this.query || this.tag || this.country)) {
          return this.data
        }
        const self = this
        let data
        ...
        if (this.country) {
          const source = data || this.data
          data = source.filter(function (item) {
            return item.country.includes(self.country)
          })
        }
        if (this.tag) {
          const source = data || this.data
      ...
      countries: function () {
        let countries = this.flatten(this.data.map(function (item) {
          return item.country.split(',')
        }))
        countries = Array.from(new Set(countries.map(function (country) {
          return country.trim()
        })))
